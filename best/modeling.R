...
fit_results<-weeks2_fit %>%
  mutate(glance = map(fit, sw_glance)) %>%
  unnest(glance, .drop = TRUE)

fit_results<-fit_results[order(fit_results$DIVISION),]

# Plot residuals ----------------------------------------------------------
augment_fit_arima <- weeks2_fit_arima %>%
  mutate(augment = map(fit, sw_augment, timetk_idx = TRUE, rename_index = "date")) %>%
  unnest(augment)

sr=unique(as.character(df$DIVISION))

pdf("division_arima_residuals_plots.pdf")
for (s in sr[2:length(sr)]){
  
    gg<-ggplot(augment_fit_arima[augment_fit_arima$DIVISION==s,],aes_string(x = "date", y = ".resid")) +
      geom_hline(yintercept = 0, color = "grey40") +
      geom_line(color = palette_light()[[2]]) +
      geom_smooth(method = "loess") +
      labs(title = paste("Division:",s),
...
arima_fcast <- weeks2_fit_arima %>%
  mutate(fcast = map(fit, forecast, h = 40))

arima_fcast_tidy <- arima_fcast %>%
  mutate(sweep = map(fcast, sw_sweep, fitted = FALSE, timetk_idx = TRUE)) %>%
  unnest(sweep)

pdf("division_arima_forecast_plots.pdf")
for (s in sr[2:length(sr)]){
  
    gg<-ggplot(arima_fcast_tidy[arima_fcast_tidy$DIVISION==s,],aes(x = index, y = total.qty, color = key)) +
  ...
  print(gg)
  htmlwidgets::saveWidget(ggplotly(gg),  paste(s,'.html', sep=""))
}
dev.off()
...
for (s in unique(result$DIVISION)) {
  ...
  gg<-ggplot(df,aes(x = index, y = total.qty, color = key)) +
    ...
    geom_line() +
    labs(title = s,
         subtitle = paste("Model Forecast:",df$model[1]) ,
         x = "", y = "Reported Incidents") +
    #scale_x_date(date_breaks = "1 quarter", date_labels = "%Y") +
    scale_x_date(date_breaks = "12 weeks", date_labels = "%Y-%m") +
    scale_color_tq() +
